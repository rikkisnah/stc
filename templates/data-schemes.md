...
| `Rule Confidence` | float | Highest confidence from the matched rule(s); only populated when `Categorization Source = rule`. |
| `LLM Confidence` | float | For rule rows, duplicates `Rule Confidence`; for `llm` rows, records the model-reported confidence. |
